          ...
          {items.map((item, index) => (
            <li key={index}>
              <a
                ...
                className="group flex items-start gap-3 min-h-[48px] py-3 px-1 -mx-1 rounded touch-manipulation border-b border-white/10 last:border-0 hover:border-(--primary)/50 hover:bg-white/5 transition-colors"
              >
                <span className="material-icons text-(--primary) text-lg mt-0.5 shrink-0 opacity-80 group-hover:opacity-100" aria-hidden>
                  arrow_forward
                </span>
                <span className="text-white/90 group-hover:text-white text-sm md:text-base leading-snug transition-colors flex-1 min-w-0">
                  {item.title}
                </span>
